...
| **Model exfiltrates source through a search query** | The central risk of web access, and the one that cannot be mitigated by filtering results: `?q=` is an outbound channel. Controls are on the outbound side — off by default; queries capped at 300 characters and a single line, so a file cannot be smuggled in one; every query and URL written to the audit log **before** the request is sent, so a crash mid-flight still leaves the record; and only hosts the user listed are reachable. This reduces and records the risk; it does not eliminate it, and enabling web access is a deliberate reversal of this extension's posture |
| Web request escapes the host allow-list via redirect | Redirects are followed manually and the allow-list is re-applied at every hop, so an approved host cannot bounce the request to an unapproved one. Same rule the model downloader already applies |
| Web access enabled but unconfigured opens everything | There is no wildcard and no "any host" value. An empty allow-list reaches nothing, so a half-finished configuration fails closed |
| Fetched page is treated as instructions | Pages are neutralized and framed as `<web_result>` exactly like workspace files, scripts and styles are stripped, and the tag is in the reserved list so a page cannot close its own wrapper |
| Cleartext or credentialed web request | HTTPS only; URLs carrying embedded credentials are refused |
