...
`v0.5.0` and the weights currently published. When either is republished the
...
```powershell
$Base = "https://github.com/Arjun10g/restricted-local-coder/releases/download/v0.5.0"
$Expected = ((Invoke-WebRequest -Uri "$Base/restricted-local-coder-0.5.0-win32-x64.vsix.sha256" -UseBasicParsing).Content -split '\s+')[0]
...
.\scripts\Invoke-SmokeTest.ps1 `
  -RuntimePath "$env:USERPROFILE\.vscode\extensions\restricted-local.restricted-local-coder-0.5.0\runtime\win32-x64\llama-server.exe" `
