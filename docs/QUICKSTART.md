...
$name = "restricted-local-coder-0.5.0-win32-x64.vsix"
...
```bash
base=https://github.com/<owner>/restricted-local-coder/releases/download/<tag>
curl -LO "$base/restricted-local-coder-0.5.0-linux-x64.vsix"
curl -LO "$base/restricted-local-coder-0.5.0-linux-x64.vsix.sha256"
```
...
npx --yes @vscode/vsce@3.9.2 package --target win32-x64 \
  --out ../artifacts/restricted-local-coder-0.5.0-win32-x64.vsix --no-dependencies
cd ..
```
...
```powershell
(Get-FileHash .\restricted-local-coder-0.5.0-win32-x64.vsix -Algorithm SHA256).Hash.ToLower()
Get-Content .\restricted-local-coder-0.5.0-win32-x64.vsix.sha256
```

```bash
shasum -a 256 restricted-local-coder-0.5.0-linux-x64.vsix
cat restricted-local-coder-0.5.0-linux-x64.vsix.sha256
```
...
```bash
code --install-extension ./vsix/restricted-local-coder-0.5.0-win32-x64.vsix
